...
sub BUILD {
    ...
    for (my $facet_id = 0; $facet_id <= $#{$self->facets}; $facet_id++) {
        ...
        {
          my $z_lowest = ($self->vertices->[$facet->[1]][Z] <= $self->vertices->[$facet->[2]][Z] &&
                          $self->vertices->[$facet->[1]][Z] <= $self->vertices->[$facet->[3]][Z]) ?
                            1 : ($self->vertices->[$facet->[2]][Z] <= $self->vertices->[$facet->[3]][Z]) ?
                                2 : 3;
            @$facet[1..3] = (@$facet[$z_lowest..3], @$facet[1..($z_lowest-1)]);
        }
